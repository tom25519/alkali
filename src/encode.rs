...
pub mod hex {
    ...
    pub fn encode(buf: &[u8]) -> Result<String, AlkaliError> {
        ...
        let hex_len = unsafe {
            ...
            // C-formatted string will be written. The second argument specifies the maximum number
            // of bytes which can be written to this pointer. We use `out.len()` to specify the
            // number of bytes which can be written, so this number of bytes can definitely be
            // written to `out`.  The next two arguments specify the buffer to encode, and its
            // length. We use `buf.len()` to specify the number of bytes to encode, so it is
            // correct for this pointer.
    ...
    ///
    /// # Safety
    /// `ignore` must be a valid C-style string, i.e: it must be null-terminated. This is best done
    /// by taking a standard Rust string, and using `CString` to convert it to the C format.
    ...
    unsafe fn decode_impl(
        ...
        // which can be written, so this number of bytes can definitely be written to `output`. The
        // next two arguments specify the hex string to decode, and its length. As the second
        // argument specifies the length of the string, it does not need to be null-terminated, so
        // we just pass the Rust byte representation of the string directly, and specify is length
        // using `.len()`. The next argument is a pointer to a C-formatted string of characters to
        // ignore. It is the responsibility of the caller to provide a valid pointer here. The next
        // argument specifies the location to which the length of the decoded output will be
        // written. We simply pass a mutable reference to a `usize` value, which is reasonable
        // here. The final argument should be a pointer to which a pointer to the final valid byte
        // of the hex string will be written. It is documented that if this is simply set to NULL,
        // Sodium will ignore it.
    ...
    /// the decoded bytes will be written.
    ///
    /// This function will return an error if `output` is insufficient to store the decoded bytes,
    /// or if `hex` contains characters which are not valid hex characters, `[0-9a-fA-F]`. If you
    /// want to decode a string containing characters which can be ignored (e.g: `"CA FE:BA BE"`),
    /// you can use [`decode_ignore`].
    ///
    ...
    ///
    /// This function will return an error if `output` is insufficient to store the decoded bytes,
    /// or if `hex` contains characters which are not valid hex characters, `[0-9a-fA-F]`, and
    /// which have not been specified to be ignored.
    ///
    /// If decoding was successful, returns the number of bytes written to `output`.
    ///
    /// This decoding runs in constant-time for a given length of hex string.
    pub fn decode_ignore(hex: &str, ignore: &str, output: &mut [u8]) -> Result<usize, AlkaliError> {
        ...
        let written = unsafe {
            ...
            // string, then calling `into_raw`. The definition of CString in the Rust standard
            // library says that this produces a valid pointer to a C-formatted string.
            decode_impl(hex, ignore_ptr, output)
        };

        // Make sure we free the ignore string's memory.
        let _ignore = unsafe {
            // SAFETY: The `ignore_ptr` pointer was created using CString::into_raw, so it is safe
            // to create a CString from it.
...
pub mod base64 {
    ...
    pub fn encode(buf: &[u8], variant: Variant) -> Result<String, AlkaliError> {
        ...
        let base64_len = unsafe {
            ...
            // written to `out`.  The next two arguments specify the buffer to encode, and its
            // length. We use `buf.len()` to specify the number of bytes to encode, so it is
            // correct for this pointer. The final argument specifies the Base64 variant to use. We
            // define the `Base64Variant` enum based on the `sodium_base64_VARIANT_NAME` constants,
            // so every item is a valid variant.
    ...
    unsafe fn decode_impl(
        ...
        // can be written to this pointer. We use `output.len()` to specify the number of bytes
        // which can be written, so this number of bytes can definitely be written to `output`. The
        // next two arguments specify the Base64 string to decode, and its length. As the second
        // argument specifies the length of the string, it does not need to be null-terminated, so
        // we just pass the Rust byte representation of the string directly, and specify is length
        // using `.len()`.  The next argument is a pointer to a C-formatted string of characters to
        // ignore. It is the responsibility of the caller to provide a valid pointer here. The next
        // argument specifies the location to which the length of the decoded output will be
        // written. We simply pass a mutable reference to a `usize` value, which is reasonable
        // here. The next argument should be a pointer to which a pointer to the final valid byte
        // of the Base64 string will be written.  It is documented that if this is simply set to
        // NULL, Sodium will ignore it. The final argument specifies the Base64 variant to use. We
        // define the `Base64Variant` enum based on the `sodium_base64_VARIANT_NAME` constants, so
        // every item is a valid variant.
    ...
    ///
    /// This function will return an error if `output` is insufficient to store the decoded bytes,
    /// or if `base64` contains characters which are not valid for this variant. If you want to
    /// decode a string containing characters which can be ignored, you can use [`decode_ignore`].
    ///
    /// If decoding was successful, returns the number of bytes written to `output`.
    ...
    ///
    /// This function will return an error if `output` is insufficient to store the decoded bytes,
    /// or if `base64` contains characters which are not valid for this variant, and have not been
    ...
    pub fn decode_ignore(
        ...
        let written = unsafe {
            ...
            // string, then calling `into_raw`. The definition of CString in the Rust standard
            // library says that this produces a valid pointer to a C-formatted string.
            decode_impl(base64, ignore_ptr, variant, output)
        };
